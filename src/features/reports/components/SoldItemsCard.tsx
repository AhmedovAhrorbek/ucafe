      ...
      {data && data.length > 0 ? (
        <ul className="flex flex-col gap-3">
          {data.map((item) => (
            <div
              key={item.id}
              className="flex flex-col sm:flex-row items-center justify-between bg-white py-4 rounded-[6px] px-4 gap-2 sm:gap-4"
            >
              <p className="w-full sm:w-[130px] font-fs-pro-display text-[14px] sm:text-[16px] font-medium leading-[19px] text-left">
                {item.name}
              </p>
              <Tag className="w-full sm:w-[150px] px-2 py-1 flex items-center gap-2 bg-white font-fs-pro-display text-[12px] sm:text-[14px] leading-[16px] text-left">
                {renderIcon(item.category)}
                {item.category}
              </Tag>
              <span className="w-full sm:w-[100px] font-fs-pro-display text-[12px] sm:text-[14px] leading-[16px] text-left text-[#2F3138]">
                {item.total_quantity} шт
              </span>
              <span className="w-full sm:w-[100px] font-fs-pro-display text-[12px] sm:text-[14px] leading-[16px] text-left text-[#2F3138]">
                {formatAmount(item.price_per_unit)} UZS
              </span>
              <span className="w-full sm:w-[165px] font-fs-pro-display text-[12px] sm:text-[14px] leading-[16px] text-left text-[#2F3138]">
        ...
        <h2 className="flex items-center justify-center font-medium text-[16px] sm:text-[20px]">
          Нет данных о проданных товарах
        </h2>
      )}
    </>
  );
